...
# Feature type name used in FDS Dataset Op.
...
class FDSDataset(dataset_ops.DatasetSource):
    """A `Dataset` comprising records from one or more Avro files.
    Input Avro files must be serialized with Quince-FDS schema.
    For more details about Quince-FDS schema, please check go/qt-fds
    ...
    FDS: Shuffle elements(1, 96) to create a batch of size 64

    Case 4: no shuffle
    Shuffle buffer size is 0
    Tensorflow and FDS both will just directly read to create a batch of size 64
    ...
    >>> # Define Avro schema in Quince-FDS format.
    >>> json_schema = '''{
    ...     "type": "record",
    ...     "name": "example",
    ...     "fields": [
    ...
    ... }
    >>> for batch in FDSDataset([example_path], batch_size=2,
    ...
    def __init__(self,
        ...
        """Creates a `FDSDataset` to read one or more Avro files encoded with
           Quince-FDS Schema.
        ...
        if features is None or not isinstance(features, dict):
            raise ValueError(
                ...
                f" and FDS feature configuration as value but found {features}."
        ...
        for key in sorted(features):
            ...
            if not isinstance(feature, _SUPPORTED_FEATURE_CONFIG):
                raise ValueError(f"Unknown FDS feature configuration {feature}. "
        ...
        variant_tensor = core_ops.io_fds_dataset(
