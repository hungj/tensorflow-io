...
FDS_SUPPORTED_DTYPES = [tf.int32, tf.int64, tf.float32, tf.float64, tf.string, tf.bool]


def _raise_error_if_dtype_not_supported(dtype: tf.dtypes.DType):
    if dtype not in FDS_SUPPORTED_DTYPES:
        raise ValueError(f"{dtype} is not supported in FDS. "
                         f"Available dtypes are {FDS_SUPPORTED_DTYPES}.")
