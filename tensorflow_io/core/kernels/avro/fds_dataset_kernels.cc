...
#include "tensorflow_io/core/kernels/avro/fds_dataset_kernels.h"
...
#include "tensorflow_io/core/kernels/avro/fds/avro_block_reader.h"
#include "tensorflow_io/core/kernels/avro/fds/errors.h"
#include "tensorflow_io/core/kernels/avro/fds/fds_decoder.h"
#include "tensorflow_io/core/kernels/avro/fds/shuffle_handler.h"
#include "tensorflow_io/core/kernels/avro/fds/decompression_handler.h"
...
/* static */ constexpr const char* const FDSDatasetOp::kDatasetType;
/* static */ constexpr const char* const FDSDatasetOp::kFileNames;
/* static */ constexpr const char* const FDSDatasetOp::kBatchSize;
/* static */ constexpr const char* const FDSDatasetOp::kDropRemainder;
/* static */ constexpr const char* const FDSDatasetOp::kReaderBufferSize;
/* static */ constexpr const char* const FDSDatasetOp::kShuffleBufferSize;
/* static */ constexpr const char* const FDSDatasetOp::kNumParallelCalls;
/* static */ constexpr const char* const FDSDatasetOp::kFeatureKeys;
/* static */ constexpr const char* const FDSDatasetOp::kFeatureTypes;
/* static */ constexpr const char* const FDSDatasetOp::kSparseDtypes;
/* static */ constexpr const char* const FDSDatasetOp::kSparseShapes;
/* static */ constexpr const char* const FDSDatasetOp::kOutputDtypes;
/* static */ constexpr const char* const FDSDatasetOp::kOutputShapes;
/* static */ constexpr const char* const FDSDatasetOp::kDenseType;
/* static */ constexpr const char* const FDSDatasetOp::kSparseType;
/* static */ constexpr const char* const FDSDatasetOp::kVarlenType;

class FDSDatasetOp::Dataset : public DatasetBase {
 public:
    ...
    for (size_t i = 0; i < num_of_features; i++) {
      if (feature_types[i] == kDenseType) {
        ...
        for (size_t d = 1; d < rank; d++) {
          shape.AddDim(dim_v[d]);
        }
        dense_features_.emplace_back(fds::FeatureType::dense,
                                     feature_keys_[i],
                                     output_dtypes[i],
                                     shape,
                                     num_of_dense_);
        ...
        if (feature_types[i] == kSparseType) {
          sparse_features_.emplace_back(fds::FeatureType::sparse,
                                        feature_keys_[i],
          ...
          varlen_features_.emplace_back(fds::FeatureType::varlen,
                                        feature_keys_[i],
                                        sparse_dtypes[num_of_sparse_],
                                        sparse_shapes[num_of_sparse_],
                                        num_of_sparse_,
 ...
 private:
   ...
   * ordered based on the layout in fds::sparse::ValueBuffer.
  ...
  class Iterator : public DatasetIterator<Dataset> {
    ...
    Status Initialize(IteratorContext* ctx) {
      ...
      if (num_threads <= 0 || num_threads > max_parallelism) {
        if (num_threads == tensorflow::data::model::kAutotune) {
          LOG(INFO) << "Thread autotuning enabled for FDSDatasetOp::Dataset::Iterator.";
        }
        LOG(INFO) << "Create FDSDatasetOp::Dataset::Iterator thread pool with "
      ...
      while(true) {
        ...
        if (count >= batch_size_ ||
          ...
          shuffle_handler_->SampleBlocks(batch_size,
                                         ...
                                         blocks_);
          std::vector<fds::sparse::ValueBuffer> sparse_buffer(num_threads, value_buffer_);

          std::vector<Status> status_of_threads(num_threads);
          auto process_block = [&](size_t i, size_t thread_idx, avro::DecoderPtr& decoder, fds::sparse::ValueBuffer& buffer,
            ...
            while (start < end) {
              ...
              auto decoding_status = fds_decoder_->DecodeFDSDatum(decoder, dense_tensors, buffer, skipped, start);
          ...
          auto process = [&](size_t index) {
            ...
            auto skipped = fds_decoder_->GetSkippedData();
            ...
            status_of_threads[index] = Status::OK();
          ...
          auto fill_sparse_value = [&](int64 thread_index) {
            ...
            for (size_t i = 0; i < num_of_sparse; i++) {
              ...
              fds::sparse::FillIndicesTensor(buffer.indices[i], indices_tensors[i],
                                             rank_after_batch * offset);
              fds::sparse::FillValuesTensor(buffer, values_tensors[i], sparse_dtypes[i],
   ...
   private:
      ...
      while(true) {
        // 1. wait for a slot in the buffer
        {
          ...
          if (cancelled_) {
            ...
            prefetch_thread_status_ = Status::OK();
            cond_var_->notify_all();
            write_var_->notify_all();
            return;
          }
    ...
    Status EnsurePrefetchThreadStarted(IteratorContext* ctx)
      ...
      if (!prefetch_thread_) {
        ...
        prefetch_thread_ = ctx->StartThread(
            "fds_data_prefetch", [this, new_ctx]() { PrefetchThread(new_ctx); });
      }
      return Status::OK();
    }
    ...
    Status SetupStreamsLocked(Env* env,
      ...
      if (fds_decoder_ == nullptr) {
        fds_decoder_ = std::make_unique<fds::FDSDecoder>(dataset()->dense_features_,
                                                         dataset()->sparse_features_,
                                                         dataset()->varlen_features_);
        TF_RETURN_IF_ERROR(fds_decoder_->Initialize(reader->GetSchema()));
        expected_schema_ = fds_decoder_->GetSchema().toJson(false);
      } else if (expected_schema_ != reader->GetSchema().toJson(false)) {
        string expected_schema = fds_decoder_->GetSchema().toJson(true);
        string varied_schema = reader->GetSchema().toJson(true);
        string filename = dataset()->filenames_[0];
        return fds::VariedSchemaNotSupportedError(expected_schema, filename,
    ...
    void InitSparseValueBuffer(fds::sparse::ValueBuffer& buffer, size_t num_of_datum) {
      auto& sparse_dtype_counts = dataset()->sparse_dtype_counts_;
    ...
    fds::sparse::ValueBuffer value_buffer_;
    ...
    std::unique_ptr<fds::FDSDecoder> fds_decoder_ = nullptr;
  ...
  std::vector<fds::dense::Metadata> dense_features_;
  std::vector<fds::sparse::Metadata> sparse_features_;
  std::vector<fds::varlen::Metadata> varlen_features_;
...
FDSDatasetOp::FDSDatasetOp(OpKernelConstruction* ctx)
    : DatasetOpKernel(ctx) {
...
void FDSDatasetOp::MakeDataset(OpKernelContext* ctx,
                               DatasetBase** output) {
...
namespace {
REGISTER_KERNEL_BUILDER(Name("IO>FDSDataset").Device(DEVICE_CPU),
                        FDSDatasetOp);
}  // namespace
