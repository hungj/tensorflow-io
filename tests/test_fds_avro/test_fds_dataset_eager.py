...
def test_FDSReader_skip_block_with_null_value(record_data, feature_name):
    writer_schema = """{
          "type": "record",
          "name": "row",
          "fields": [
...
def test_FDS_dataset_creation_with_invalid_argument(filenames, batch_size,
...
@pytest.mark.parametrize(
    ["filenames", "batch_size", "features", "error_message"], [
        ...
        (
            ...
            r"Unknown FDS feature configuration \(\[\], tf\.int32\)\. Only .* are supported\."),
        (
            ["filename"], 1, {},
        ...
        (
            ...
            r"FDS feature configuration as value but found None\. "
            r"Available feature configuration are .*"),
        (
            ["filename"], 1, ([], tf.int32),
            r"Features can only be a dict with feature name as key and "
            r"FDS feature configuration as value but found \(\[\], tf\.int32\)\. "
            r"Available feature configuration are .*"),
    ]
)
def test_FDS_dataset_creation_with_value_error(filenames, batch_size,
...
def test_FDS_dataset_with_null_value(record_data, feature_name):
    writer_schema = """{
          "type": "record",
          "name": "row",
          "fields": [
    ...
    error_message = f"Failed to decode feature {feature_name}. " \
                    f"Reason: Feature value is null."
    with pytest.raises(errors.InvalidArgumentError, match=error_message):
        dataset = FDSDataset(filename, features=features, batch_size=2)
        iterator = iter(dataset)
        next(iterator)
...
def test_valid_shuffle(shuffle_buffer_size, batch_size, num_parallel_calls):
    ...
    for i in range(0, num_epochs):
        li = list_from_dataset(FDSDataset(
            filenames=filename,
            features=features,
            shuffle_buffer_size=shuffle_buffer_size,
            batch_size=batch_size,
...
def test_empty_sparse_buffer():
    ...
    # FDSReader is parallelized along blocks.
...
class FDSDatasetTest(AvroDatasetTestBase):
    """FDSDatasetTest"""
    ...
    def test_FDS_dataset_with_interleave(self):
        writer_schema = """{
              "type": "record",
              "name": "row",
              "fields": [
        ...
        dataset = dataset.interleave(
            lambda x: FDSDataset(x, features=features, batch_size=2))
        self._verify_output(expected_data=expected_data, actual_dataset=dataset)

    def test_FDS_dataset_with_file_not_existed(self):
    ...
    def test_FDS_dataset_with_feature_not_existed(self):
        writer_schema = """{
              "type": "record",
              "name": "row",
              "fields": [
        ...
        with pytest.raises(errors.InvalidArgumentError, match=error_message):
            dataset = FDSDataset(filenames, features=features, batch_size=2)
            iterator = iter(dataset)
            next(iterator)

    def test_FDS_dataset_with_null_schema(self):
        writer_schema = """{
              "type": "record",
              "name": "row",
              "fields": [
    ...
    def test_FDS_dataset_with_multithreading(self):
        writer_schema = """{
              "type": "record",
              "name": "row",
              "fields": [
    ...
    def test_FDS_dataset_processing_multiple_files(self):
        writer_schema = """{
              "type": "record",
              "name": "row",
              "fields": [
    ...
    def test_FDS_dataset_processing_multiple_files_with_different_schema(self):
    ...
    def test_profiler_traceme(self):
        """Test FDSDataset exposing metric through tf.profiler traceme."""
        data_size = 128
        data_dimension = 128
        writer_schema = """{
            "type": "record",
        ...
        record_data = [
            {
                "float_1d": np.random.random(data_dimension).tolist()
            } for _ in range(0, data_size)
        ]

        features = {
            "float_1d": DenseFeature([data_dimension], tf.dtypes.float32),
        }
        dataset = create_fds_dataset(
            writer_schema=writer_schema,
            record_data=record_data,
            features=features,
            # Make sure batch size is large enough to have more than 2 blocks.
            batch_size=128,
            codec="deflate",
            num_parallel_calls=2
        )

        # Expected metrics exposed by FDSDataset
        metric_names = [
            "WaitingForData", "ParsingThread_0", "ParsingThread_1",
            "FillingSparseValues", "BlockReading", "DeflateDecompression"
        ]
        with tempfile.TemporaryDirectory() as tmpdir:
            tf.profiler.experimental.start(tmpdir)
            for _ in dataset:
                pass
            tf.profiler.experimental.stop()

            found_trace_event = False
            assert os.path.exists(tmpdir)
            for root, dirs, files in os.walk(tmpdir):
                for filename in files:
                    # FDSDataset only exposes trace event.
                    if filename.endswith("trace.json.gz"):
                        trace_path = os.path.join(root, filename)
                        with gzip.open(trace_path, 'rb') as f:
                            trace = json.load(f)

                        trace_event_names = set()
                        for trace_event in trace['traceEvents']:
                            if 'name' in trace_event:
                                trace_event_names.add(trace_event['name'])

                        for metric_name in metric_names:
                            assert metric_name in trace_event_names
                        found_trace_event = True
            assert found_trace_event
