...
@pytest.mark.parametrize(
    ["shape", "dtype", "error_message"], [
        ...
        ([1, 2], tf.float16, f"<dtype: 'float16'> is not supported in FDS. Available dtypes are {FDS_SUPPORTED_DTYPES}."),
        ([-1, 2], tf.int32, "Each dimension should be greater than 0 in DenseFeature but found [-1, 2]."),
    ]
)
def test_fds_dense_feature(shape, dtype, error_message):
...
def test_fds_sparse_feature(shape, dtype, error_message):
...
def test_fds_ragged_feature(shape, dtype, error_message):
