...
def test_hash_code():
  writer = FDSWriter(codec="null")
  null_codec_writer = FDSWriter(codec="null")
  deflate_codec_writer = FDSWriter(codec="deflate")
  snappy_codec_writer = FDSWriter(codec="snappy")
...
def test_read_from_tf_record_cache(generator, num_records, partitions):
  ...
  try:
    ...
    with FDSWriter() as fds_writer, TFRecordWriter() as tf_writer:
      fds_path = fds_writer.write(data_source)
      tf_path = os.path.join(fds_path, os.pardir, tf_writer.hash_code())
      ...
      for file_index in range(partitions):
        ...
        fds_filename = os.path.join(fds_path, f"part-{index_name}.{fds_writer.extension}")
        tf_filename = os.path.join(tf_path, f"part-{index_name}.{tf_writer.extension}")
        fds_dataset = get_dataset(fds_filename, get_features_from_data_source(fds_writer, data_source))
        fds_dataset = fds_dataset.unbatch()
        ...
        for fds_record, tf_record in zip(fds_dataset, tf_dataset):
          actual = tf.cast(tf_record[feature_name], dtype)
          if dtype in [tf.float32, tf.float64]:
            tf.debugging.assert_near(actual, fds_record[feature_name], atol=1e-6)
            tf.debugging.assert_near(data[count], fds_record[feature_name], atol=1e-6)
          else:
            tf.debugging.assert_equal(actual, fds_record[feature_name])
            tf.debugging.assert_equal(data[count], fds_record[feature_name])
          count += 1
